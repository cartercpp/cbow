...
#ifndef CBOW_MATRIX_H
#define CBOW_MATRIX_H
...
public:
    ...
    matrix(std::initializer_list<std::initializer_list<ValueType>> data)
        : m_data{}, m_rows{data.size()}, m_columns{(data.begin() != data.end()) ? data.begin()->size() : 0}
    {
        ...
        for (const auto& row : data)
        {
            if (row.size() != m_columns)
                ...
                "Each row in a matrix must have the same # of columns"
            };

            m_data.emplace_back(std::vector<ValueType>{row});
        }
    }

    matrix(const std::vector<math_vector<ValueType>>& data)
        : m_data{data}, m_rows{data.size()}, m_columns{!data.empty() ? data[0].size() : 0}
    {
